...
data_long <- data_long %>%
  arrange(Page, date) %>%
  group_by(Page) %>%
  mutate(lag_1 = lag(page_visits, n = 1),
         lag_7 = lag(page_visits, n = 7))
...
data_long <- data_long %>%
  arrange(Page, date) %>%
  group_by(Page) %>%
  mutate(roll_mean_7 = rollmean(page_visits, 7, fill = NA),
         roll_std_7 = rollapply(page_visits, 7, sd, fill = NA))
...
data_long <- data_long %>%
  arrange(Page, date) %>%
  group_by(Page) %>%
  mutate(growth_rate = (page_visits - lag(page_visits, n = 1)) / lag(page_visits, n = 1) * 100)
...
```{r plot features from graph}
# Don't use this chunk, still trying out things
filtered_wiki_df_with_dates_country_device %>%
  filter(Page == "Michael_J._Fox_en.wikipedia.org_all-access_all-agents") %>%
  pivot_longer(cols = -c(Page, name, language, device), names_to = 'Date', values_to = 'Web_Traffic') %>%
  mutate(Date = as.Date(Date, format = '%Y-%m-%d')) %>%
  ggplot(aes(x = language, y = Web_Traffic)) +
  geom_boxplot()

unique(filtered_wiki_df_with_dates_country_device$language)
en_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "en", 5:869])  
de_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "de", 5:869])
es_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "es", 5:869])
ja_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "ja", 5:869])
ru_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "ru", 5:869])
zh_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "zh", 5:869])
fr_l <- colMeans(filtered_wiki_df_with_dates_country_device[filtered_wiki_df_with_dates_country_device$language == "fr", 5:869])

all_dates <- filtered_wiki_df_with_dates_country_device %>%
  filter(Page == "Michael_J._Fox_en.wikipedia.org_all-access_all-agents") %>%
  pivot_longer(cols = -c(Page, name, language, device), names_to = 'Date', values_to = 'Web_Traffic') %>%
  mutate(Date = as.Date(Date, format = '%Y-%m-%d')) %>%
  select(Date)
...
# Create data frame
language_df <- data_frame(Dates = all_dates$Date,
                          en = en_l,
                          de = de_l,
                          es = es_l,
                          ja = ja_l,
